...
export default class AppUpdater {
  ...
  private async _getPreReleaseVersionFromGithub(channel: UpgradeChannel) {
    try {
      ...
      const responses = await fetch('https://api.github.com/repos/CherryHQ/SwiftEZ/releases?per_page=8', {
        headers: {
      ...
      return `https://github.com/CherryHQ/SwiftEZ/releases/download/${release.tag_name}`
    } catch (error) {
